...
button_pin_1 = 26  # DRDY chip 1
button_pin_2 = 13  # DRDY chip 2
cs_pin = 19

chip = gpiod.chip("0")

# CS line for chip 2 (manual)
...
# DRDY line chip 1 (GPIO26) — input
drdy_line_1 = chip.get_line(button_pin_1)
drdy_cfg_1 = gpiod.line_request()
drdy_cfg_1.consumer = "DRDY1"
drdy_cfg_1.request_type = gpiod.line_request.DIRECTION_INPUT
drdy_line_1.request(drdy_cfg_1)

# DRDY line chip 2 (GPIO13) — input
drdy_line_2 = chip.get_line(button_pin_2)
drdy_cfg_2 = gpiod.line_request()
drdy_cfg_2.consumer = "DRDY2"
drdy_cfg_2.request_type = gpiod.line_request.DIRECTION_INPUT
drdy_line_2.request(drdy_cfg_2)

def wait_for_drdy(line):
    while line.get_value() == 0:  # wait if still low from previous read
        pass
    while line.get_value() == 1:  # wait for HIGH -> LOW falling edge
        pass
...
data_after = []
just_one_time = 0
data_lenght_for_Filter = 2
read_data_lenght_one_time = 1
sample_len = 250
sample_lens = 250
fps = 250
highcut = 1
...
data_before_8  = [0] * 250
data_before_9  = [0] * 250
data_before_10 = [0] * 250
data_before_11 = [0] * 250
data_before_12 = [0] * 250
data_before_13 = [0] * 250
data_before_14 = [0] * 250
data_before_15 = [0] * 250
data_before_16 = [0] * 250
...
while 1:
    ...
    # ── DRDY: wait for falling edge on chip 1, then read ─────────────────────
    wait_for_drdy(drdy_line_1)
    output = spi.readbytes(27)

    # ── DRDY: wait for falling edge on chip 2, then read ─────────────────────
    wait_for_drdy(drdy_line_2)
    cs_line.set_value(0)
